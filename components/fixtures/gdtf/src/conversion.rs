...
impl From<GdtfFixtureDefinition> for FixtureDefinition {
    fn from(definition: GdtfFixtureDefinition) -> Self {
        ...
        FixtureDefinition {
            ...
            tags: Vec::new(),
            modes: definition.fixture_type.dmx_modes.modes.into_iter()
                .map(|mode| state.build_fixture_mode(mode)).collect(),
...
impl GdtfState {
    fn build_state(definition: &GdtfFixtureDefinition) -> Self {
        ...
        for attribute in &definition.fixture_type.attribute_definitions.attributes.attributes {
            attributes.insert(attribute.name.clone(), attribute.clone());
        }
        for feature_group in &definition.fixture_type.attribute_definitions.feature_groups.groups {
    ...
    fn build_fixture_mode(&self, mode: DmxMode) -> FixtureMode {
            ...
            if let Some(attribute) = self.attributes.get(&channel.logical_channel.attribute) {
                if let Some(feature) = self.features.get(&attribute.feature) {
                    ...
                    match feature.name.as_str() {
                        ...
                        "PanTilt" => {
                            match attribute.name.as_str() {
                                "Pan" => controls.pan = Some(AxisGroup {
                                    channel,
                                    angle: None,
                                }),
                                "Tilt" => controls.tilt = Some(AxisGroup {
                                    channel,
                                    angle: None,
                                }),
                                _ => {}
        ...
        if let (Some(red), Some(green), Some(blue)) = (color_builder.red, color_builder.green, color_builder.blue) {
        ...
        FixtureMode {
            ...
            channels: mode.channels.channels.into_iter()
