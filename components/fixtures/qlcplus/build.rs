...
fn main() {
    ...
    let mut archive = ureq::get("https://github.com/mcallegari/qlcplus/archive/refs/heads/master.zip")
        .call()
        .unwrap()
        .into_reader();
    ...
    let fixtures_folder_path = download_dir.path().join("qlcplus-master").join("resources").join("fixtures");
    ...
    let fixture_folders = fixture_folders
        ...
        .filter(|entry| entry.metadata().ok().map(|metadata| metadata.is_dir()).unwrap_or_default())
        .map(|entry| entry.path())
        .collect::<Vec<_>>();

    fs_extra::copy_items(&fixture_folders, &target_path, &CopyOptions {
        skip_exist: true,
        overwrite: false,
        copy_inside: true,
        depth: 2,
