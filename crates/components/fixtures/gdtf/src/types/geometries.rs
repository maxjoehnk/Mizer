...
impl GeometryType {
    ...
    fn first_beam(&self) -> Option<&Beam> {
        match self {
            GeometryType::Geometry(geometry) => geometry.children.iter().find_map(|g| g.first_beam()),
...
impl Beam {
    pub fn child_beams(&self) -> Vec<&Beam> {
        self.children.iter().flat_map(|g| match g {
    ...
    pub fn child_names(&self) -> Vec<&str> {
        self.children.iter().flat_map(|g| match g {
            GeometryType::Beam(beam) => vec![vec![beam.name.as_str()], beam.child_names()].concat(),
            GeometryType::Geometry(geometry) => vec![vec![geometry.name.as_str()], geometry.child_names()].concat(),
        }).collect()
    }
}
