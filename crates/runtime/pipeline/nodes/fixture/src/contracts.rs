...
pub(crate) trait FixtureController {
    fn write_group_control(&self, group_id: GroupId, control: FixtureFaderControl, value: f64, priority: FixturePriority);
    fn write_fixture_control(&self, fixture_id: FixtureId, control: FixtureFaderControl, value: f64, priority: FixturePriority);
...
impl FixtureController for FixtureManager {
    ...
    fn get_group_fixture_ids(&self, group_id: GroupId) -> Vec<Vec<FixtureId>> {
        ...
        self.get_group(group_id).map(|g| g.fixtures.iter().map(|f| vec![*f]).collect::<Vec<_>>()).unwrap_or_default()
    }
}