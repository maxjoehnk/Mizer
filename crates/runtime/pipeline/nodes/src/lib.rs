...
pub use mizer_fixture_nodes::{
    FixtureControlNode, FixtureNode, GroupControlNode, GroupNode, PresetNode, ProgrammerNode, GroupMasterNode,
};
...
pub use mizer_input_nodes::{
    ButtonGridNode, ButtonNode, DialNode, FaderNode, LabelNode, TimecodeNode,
};
...
use mizer_node::{
    ConfigurableNode, DebugUiDrawHandle, ReadOnlyInjectionScope, NodeDetails, NodeSetting, NodeTemplate,
...
macro_rules! node_impl {
    ($($node_type:ident($node:ty),)*) => {
        ...
        impl Node {
            ...
            pub fn list_ports(&self, injector: &ReadOnlyInjectionScope) -> Vec<(PortId, PortMetadata)> {
                match self {
            ...
            pub fn settings(&self, injector: &ReadOnlyInjectionScope) -> Vec<NodeSetting> {
                match self {
