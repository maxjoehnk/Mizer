...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    for (var i: i32 = 0; i < count; i++) {
        ...
        rgb = blendLighten(rgb, pixel.rgb, pixel.a);
    }

    return vec4<f32>(rgb, 1.0);
}

fn lightenChannel(base: f32, blend: f32) -> f32 {
	return max(blend, base);
}

fn lighten(base: vec3<f32>, blend: vec3<f32>) -> vec3<f32> {
	return vec3(lightenChannel(base.r, blend.r), lightenChannel(base.g, blend.g), lightenChannel(base.b, blend.b));
}
