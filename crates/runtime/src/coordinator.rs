...
fn register_node(pipeline: &mut PipelineWorker, path: NodePath, node: Node, injector: &Injector) {
    match node {
        Node::Clock(node) => pipeline.register_node(path, &node, injector),
        Node::Oscillator(node) => pipeline.register_node(path, &node, injector),
        Node::DmxOutput(node) => pipeline.register_node(path, &node, injector),
        Node::Scripting(node) => pipeline.register_node(path, &node, injector),
        Node::Sequence(node) => pipeline.register_node(path, &node, injector),
        Node::Envelope(node) => pipeline.register_node(path, &node, injector),
        Node::Select(node) => pipeline.register_node(path, &node, injector),
        Node::Merge(node) => pipeline.register_node(path, &node, injector),
        Node::Threshold(node) => pipeline.register_node(path, &node, injector),
        Node::Encoder(node) => pipeline.register_node(path, &node, injector),
        Node::Fixture(node) => pipeline.register_node(path, &node, injector),
        Node::Programmer(node) => pipeline.register_node(path, &node, injector),
        Node::Group(node) => pipeline.register_node(path, &node, injector),
        Node::Preset(node) => pipeline.register_node(path, &node, injector),
        Node::Sequencer(node) => pipeline.register_node(path, &node, injector),
        Node::IldaFile(node) => pipeline.register_node(path, &node, injector),
        Node::Laser(node) => pipeline.register_node(path, &node, injector),
        Node::Fader(node) => pipeline.register_node(path, &node, injector),
        Node::Button(node) => pipeline.register_node(path, &node, injector),
        Node::Label(node) => pipeline.register_node(path, &node, injector),
        Node::MidiInput(node) => pipeline.register_node(path, &node, injector),
        Node::MidiOutput(node) => pipeline.register_node(path, &node, injector),
        Node::OpcOutput(node) => pipeline.register_node(path, &node, injector),
        Node::PixelPattern(node) => pipeline.register_node(path, &node, injector),
        Node::PixelDmx(node) => pipeline.register_node(path, &node, injector),
        Node::OscInput(node) => pipeline.register_node(path, &node, injector),
        Node::OscOutput(node) => pipeline.register_node(path, &node, injector),
        Node::VideoFile(node) => pipeline.register_node(path, &node, injector),
        Node::VideoColorBalance(node) => pipeline.register_node(path, &node, injector),
        Node::VideoOutput(node) => pipeline.register_node(path, &node, injector),
        Node::VideoEffect(node) => pipeline.register_node(path, &node, injector),
        Node::VideoTransform(node) => pipeline.register_node(path, &node, injector),
        Node::ColorConstant(node) => pipeline.register_node(path, &node, injector),
        Node::ColorBrightness(node) => pipeline.register_node(path, &node, injector),
        Node::ColorRgb(node) => pipeline.register_node(path, &node, injector),
        Node::ColorHsv(node) => pipeline.register_node(path, &node, injector),
        Node::Gamepad(node) => pipeline.register_node(path, &node, injector),
        Node::Container(node) => pipeline.register_node(path, &node, injector),
        Node::Math(node) => pipeline.register_node(path, &node, injector),
        Node::MqttInput(node) => pipeline.register_node(path, &node, injector),
        Node::MqttOutput(node) => pipeline.register_node(path, &node, injector),
        Node::NumberToData(node) => pipeline.register_node(path, &node, injector),
        Node::DataToNumber(node) => pipeline.register_node(path, &node, injector),
        Node::Value(node) => pipeline.register_node(path, &node, injector),
        Node::Extract(node) => pipeline.register_node(path, &node, injector),
        Node::PlanScreen(node) => pipeline.register_node(path, &node, injector),
        Node::Delay(node) => pipeline.register_node(path, &node, injector),
        Node::Ramp(node) => pipeline.register_node(path, &node, injector),
        Node::Noise(node) => pipeline.register_node(path, &node, injector),
        Node::Transport(node) => pipeline.register_node(path, &node, injector),
        Node::G13Input(node) => pipeline.register_node(path, &node, injector),
        Node::G13Output(node) => pipeline.register_node(path, &node, injector),
        Node::ConstantNumber(node) => pipeline.register_node(path, &node, injector),
        Node::Conditional(node) => pipeline.register_node(path, &node, injector),
        Node::TimecodeControl(node) => pipeline.register_node(path, &node, injector),
        Node::TimecodeOutput(node) => pipeline.register_node(path, &node, injector),
        Node::AudioFile(node) => pipeline.register_node(path, &node, injector),
        Node::AudioOutput(node) => pipeline.register_node(path, &node, injector),
        Node::AudioVolume(node) => pipeline.register_node(path, &node, injector),
        Node::AudioInput(node) => pipeline.register_node(path, &node, injector),
        Node::AudioMix(node) => pipeline.register_node(path, &node, injector),
        Node::AudioMeter(node) => pipeline.register_node(path, &node, injector),
        Node::Template(node) => pipeline.register_node(path, &node, injector),
        Node::TestSink(node) => pipeline.register_node(path, &node, injector),
    }
}
