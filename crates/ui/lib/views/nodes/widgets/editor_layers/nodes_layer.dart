    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        return Consumer<NodeEditorModel>(builder: (context, model, _) {
          final viewport = calculateViewport(constraints.biggest, model.transformationController.value, inflate: 200);

          return SizedBox.expand(
              child: Stack(
            clipBehavior: Clip.none,
            children: model.nodes.where((node) => !node.node.designer.hidden).where((node) {
              final nodeRect = Rect.fromLTWH(node.offset.dx, node.offset.dy, NODE_BASE_WIDTH, 200);
                ...
                child: GestureDetector(
                    ...
                    onPanEnd: (e) {
                      ...
                      if (model.selectedNode != null) {
                        ...
                        movements.add(
                            MoveNode(model.selectedNode!.node.path, model.selectedNode!.getPosition()));
                      }
                      for (var selectedNode in model.otherSelectedNodes) {
                        ...
                        var node = model.nodes
                            .firstWhere((element) => element.node.path == selectedNode.node.path);
...
Rect calculateViewport(Size size, Matrix4 transformMatrix, { double? inflate }) {
  final inverse = Matrix4.inverted(transformMatrix);

  final topLeft = MatrixUtils.transformPoint(inverse, Offset.zero);
  final bottomRight = MatrixUtils.transformPoint(inverse, Offset(size.width, size.height));
  ...
  return viewportRect.inflate(inflate);
}
