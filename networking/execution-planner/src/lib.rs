...
mod tests {
    ...
    fn single_executor_single_node(node_path: &str, executor_id: &str) {
        ...
        planner.nodes.insert(node.path.clone(), node.clone());
    ...
    fn multiple_executors_one_node_each() {
        ...
        planner.nodes.insert(node1.path.clone(), node1.clone());
        planner.nodes.insert(node2.path.clone(), node2.clone());
        planner.add_executor(Executor {
            id: "executor1".into(),
        });
        planner.add_executor(Executor {
