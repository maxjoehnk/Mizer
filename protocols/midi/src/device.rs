...
use midir::{MidiInputConnection, MidiOutputConnection, MidiInput, MidiOutput};
...
impl MidiDevice {
    pub(crate) fn new(identifier: MidiDeviceIdentifier) -> anyhow::Result<Self> {
        ...
        if let Some(port) = identifier.input {
            ...
            let connection = input.connect(&port, "mizer", move |time, data, _| {
                tx.send(MidiEvent {
                    timestamp: time,
        ...
        if let Some(port) = identifier.output {
            ...
            let connection = output.connect(&port, "mizer").map_err(|_| anyhow::anyhow!("opening output failed"))?;
