...
impl<T, S: 'static> ProcessingNodeExt for T where T: ProcessingNode<State = S> {
    fn process(&self, context: &PipelineContext, state: &mut Box<dyn Any>) -> anyhow::Result<()> {
...
impl PipelineWorker {
    ...
    pub fn register_node<T: 'static + ProcessingNode<State = S>, S: 'static>(&mut self, path: NodePath, node: &T) {
        let state = node.create_state();
        let state: Box<S> = Box::new(state);
        self.states.insert(path, state);
    }

    pub fn connect_nodes(&mut self, link: NodeLink, source_meta: PortMetadata, target_meta: PortMetadata) {
    ...
    fn connect_ports(&mut self, link: NodeLink, source_meta: PortMetadata, target_meta: PortMetadata) {
        match link.port_type {
            PortType::Single => self.connect_memory_ports::<f64>(link, source_meta, target_meta),
            PortType::Multi => self.connect_memory_ports::<Vec<f64>>(link, source_meta, target_meta),
            PortType::Laser => self.connect_memory_ports::<Vec<LaserFrame>>(link, source_meta, target_meta),
            PortType::Gstreamer => self.connect_gst_ports(link),
            _ => unimplemented!()
        }
    }

    fn connect_memory_ports<V: PortValue + Default + 'static>(&mut self, link: NodeLink, source_meta: PortMetadata, target_meta: PortMetadata) {
        let (tx, rx) = mizer_ports::memory::channel::<V>();
        if !self.senders.contains_key(&link.source) {
            self.senders.insert(link.source.clone(), NodeSenders::default());
        }
        ...
        if !self.receivers.contains_key(&link.target) {
            self.receivers.insert(link.target.clone(), NodeReceivers::default());
        }
    ...
    pub fn process<'a>(&mut self, mut nodes: Vec<(&'a NodePath, &'a Box<dyn ProcessingNodeExt>)>, frame: ClockFrame, injector: &Injector) {
        self.order_nodes_by_dependencies(&mut nodes);
        self.process_nodes(&mut nodes, frame, &injector)
    }

    fn order_nodes_by_dependencies(&mut self, nodes: &mut Vec<(&NodePath, &Box<dyn ProcessingNodeExt>)>) {
    ...
    fn process_nodes(&mut self, nodes: &mut Vec<(&NodePath, &Box<dyn ProcessingNodeExt>)>, frame: ClockFrame, injector: &Injector) {
