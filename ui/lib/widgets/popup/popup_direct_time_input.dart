...
class _ValueAst {
  ...
  _ValueAst(this.nodes, { this.isBeats = false });
  ...
  factory _ValueAst.fromTimer(CueTime? time) {
    ...
    return _ValueAst([_Value(value.truncate(), fractions: getFraction(value, 2))], isBeats: time.hasBeats());
  }
...
class PopupDirectTimeInput extends StatefulWidget {
  final void Function(CueTime?)? onEnter;
  final CueTime? time;

  const PopupDirectTimeInput({this.onEnter, this.time, Key? key}) : super(key: key);

  @override
  State<PopupDirectTimeInput> createState() => _PopupDirectTimeInputState();
}

class _PopupDirectTimeInputState extends State<PopupDirectTimeInput> {
  late final Map<LogicalKeyboardKey, void Function(BuildContext)> keyMappings;

  late _ValueAst _ast;

  _PopupDirectTimeInputState() {
    keyMappings = {
...
class _NumberPad extends StatelessWidget {
  ...
  const _NumberPad(this.number, this.onTap, {Key? key}) : super(key: key);
